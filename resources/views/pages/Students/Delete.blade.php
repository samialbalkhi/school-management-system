        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('Students.destroy',$student->id)}}" method="post">
                    ...
                    <h5 style="font-family: 'Cairo', sans-serif;">{{trans('StudentsTranslation.Deleted_Student')}}</h5>
                    <input type="text" readonly value="{{$student->name}}" class="form-control">

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('GradesTranslation.Close')}}</button>
                        <button  class="btn btn-danger">{{trans('GradesTranslation.Save')}}</button>
                    </div>
                </form>
            </div>
        </div>
