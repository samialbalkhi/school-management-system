        ...
        <div class="col-md-12 mb-30">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    <form action="{{route('feesinvoices.update',$fee_invoices->id)}}" method="post" autocomplete="off">
                        @method('PUT')
                        @csrf
                        <div class="form-row">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col">
                                ...
                                <select class="custom-select mr-sm-2" name="fee_id">
                                    @foreach($fee as $fees)
                                        <option value="{{$fees->id}}" {{$fees->id == $fee_invoices->fee_id ? 'selected':"" }}>{{$fees->title}}</option>
                                    @endforeach
                                </select>
                            </div>
